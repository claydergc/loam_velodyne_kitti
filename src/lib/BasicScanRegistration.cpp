...
void BasicScanRegistration::processScanlines(const float& scanTime, std::vector<pcl::PointCloud<pcl::PointXYZI>> const& laserCloudScans)
{
  ...
  //std::cout<<"laser cloud size: "<<_laserCloud.size()<<std::endl;
...
void BasicScanRegistration::reset(const float& scanTime)
{
  _scanTime = scanTime;
